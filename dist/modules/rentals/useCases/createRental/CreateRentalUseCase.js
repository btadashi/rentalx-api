...
let CreateRentalUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (carUnavailable) {
      throw new _AppErrors.AppError("Car is unavailable");
    }

    const rentalOpenToUser = await this.rentalsRepository.findOpenRentalByUser(user_id);

    if (rentalOpenToUser) {
      throw new _AppErrors.AppError("There is already a rental in progress for this user");
    }
    ...
    if (compare < minimumHour) {
      throw new _AppErrors.AppError("Invalid return time");
    }
